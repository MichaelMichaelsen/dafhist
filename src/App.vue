      ...
      .then(
        res => {
                  ...
                  for (var i in res.data) {
                    ...
                    let virkningFraIndex     = this.getTimeStampIndex(res.data[i].virkningFra)
                    let virkningTilIndex     = this.getTimeStampIndex(res.data[i].virkningTil)
                    let registreringFraIndex = this.getTimeStampIndex(res.data[i].registreringFra)
                    let registreringTilIndex = this.getTimeStampIndex(res.data[i].registreringTil)
                    let timestampVector      = {
                      id : i,
                      status: status,
                      virkningFra     : { timestamp: res.data[i].virkningFra,
                                          position: virkningFraIndex  },
                      virkningTil     : { timestamp: res.data[i].virkningTil,
                                          position: virkningTilIndex  },
                      registreringFra : { timestamp: res.data[i].registreringFra,
                                          position: registreringFraIndex  },
                      registreringTil : { timestamp: res.data[i].registreringTil,
                  ...
                  var xAxisTop    = d3.axisTop(x).ticks(maxxlength).tickFormat((d,i) => this.timeLabels(i));
                  var xAxisBottom = d3.axisBottom(x).ticks(maxxlength).tickFormat((d,i) => this.timeLabels(i));
                  ...
                  svg
                   ...
                   svg
                     ...
                     .enter()
                        .append("line")
                          .attr("x1", function(d){ return x(d.virkningFra.timestamp ? d.virkningFra.position+1 : 0) })
                          .attr("y1", function(d){ return y(+d.id+lineBeforeDist) })
                          .attr("x2", function(d){ return x(d.virkningTil.timestamp ? d.virkningFra.position+1 : maxxlength) })
                          .attr("y2", function(d){ return y(+d.id+lineBeforeDist) })
                    ...
                    svg
                      ...
                      .enter()
                         .append("line")
                           .attr("x1", function(d){ return x(d.registreringFra.timestamp ? d.registreringFra.position+1 : 0) })
                           .attr("y1", function(d){ return y(+d.id+lineAfterDist) })
                           .attr("x2", function(d){ return x(d.registreringTil.timestamp ? d.registreringTil.position+1 : maxxlength) })
                           .attr("y2", function(d){ return y(+d.id+lineAfterDist) })
                           .attr("stroke-width", strokeWidth)
                           .attr("stroke", "red");
                    ...
                    svg
                      ...
                      .enter()
                         .append("text")
                            .attr("x",function(d){ return x(d.registreringFra.timestamp ? d.registreringFra.position+1 : 1) })
                            .attr("y", function(d){ return y(+d.id+1) })
                            .text(function(d,i){ return (i+1) + ". " + darStatus(d.status)})
